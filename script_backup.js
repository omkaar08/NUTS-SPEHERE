...
let cart = JSON.parse(localStorage.getItem('nutssphere-cart')) || [];
let cartCount = 0;
...
function addToCart(productId, productName, price, weight) {
...
function getProductImage(productId) {
    const imageMap = {
        'roasted-pistachios': 'images/IMG-20250807-WA0013.jpg',
        'california-almonds': 'images/IMG-20250807-WA0014.jpg',
        'black-raisins': 'images/IMG-20250807-WA0015.jpg',
        'whole-cashews': 'images/IMG-20250807-WA0016.jpg',
        'inshell-walnuts': 'images/IMG-20250807-WA0017.jpg'
    };
...
function increaseQuantity(productId) {
    ...
    qtyElement.textContent = Math.min(currentQty + 1, 10);
}

function decreaseQuantity(productId) {
    const qtyElement = document.getElementById(`qty-${productId}`);
    const currentQty = parseInt(qtyElement.textContent);
    qtyElement.textContent = Math.max(currentQty - 1, 1);
}
        ...
        cartHTML += `
            <div class="cart-item">
                ...
                <div class="cart-item-details">
                    ...
                    <div class="cart-item-price">₹${item.price} (${item.weight}g)</div>
                    <div class="cart-item-quantity">
                        <button class="qty-btn" onclick="updateCartQuantity(${index}, -1)">-</button>
                        <span class="qty-display">${item.quantity}</span>
                        <button class="qty-btn" onclick="updateCartQuantity(${index}, 1)">+</button>
                        <button class="qty-btn" onclick="removeFromCart(${index})" style="margin-left: 10px; background-color: #f44336;">×</button>
                    </div>
                </div>
            </div>
        `;
...
function saveCart() {
    localStorage.setItem('nutssphere-cart', JSON.stringify(cart));
}
...
function proceedToCheckout() {
    ...
    let message = "Hello NutsSphere! I would like to order the following items:\n\n";
    let total = 0;
    
    cart.forEach(item => {
        const itemTotal = item.price * item.quantity;
        total += itemTotal;
        message += `• ${item.name} (${item.weight}g) - Qty: ${item.quantity} - ₹${itemTotal}\n`;
    });
    
    message += `\nTotal Amount: ₹${total.toLocaleString('en-IN')}\n\nPlease confirm the order and delivery details.`;
    
    const whatsappUrl = `https://wa.me/917038979754?text=${encodeURIComponent(message)}`;
